...
def showText2():
    ...
    if numberofRubbish.harmfulBin >= 6:
        flag3 = '已满载'
...
def test():
    randow_name =['苹果','橘子','梨','西兰花', '萝卜','葡萄','青菜','番茄','易拉罐','电池']
    randow_list = ["厨余垃圾","有害垃圾","可回收垃圾", "其他垃圾"]
    for i in range(10):
        Main_text.COUNT += 1
        Main_text.NAME = randow_name[i]
        Main_text.CATEGOARY =randow_list[i%3]
        showText(Main_text)
        time.sleep(3)
...
text1 = tk.Text(win, width=27, height=21, font=('Times', 15, 'bold')) # mark ,width 已加倍

...
ChineseName = {'apple': '苹果', 'orange': '橘子', 'pear': '梨', 'broccoli': '西兰花', 'carrot': '萝卜', 'potato': '土豆',\
               ...
               'pea': '豌豆', 'multpea': '豌豆', 'onion': '洋葱', 'whiteradish': '萝卜', 'hmg': '哈密瓜', 'longyan': '龙眼',\
...
# class detector_config_for_All:
#     def __init__(self):
#         self.weightsPath = '../weights/my-v3.backup'  # 模型权重文件
#         self.configPath = "../weights/my-v3.cfg"  # 模型配置文件
#         self.labelsPath = "../weights/classes.names"  # 模型类别标签文件
# class detector_config_just_blank:
#     def __init__(self):
#         self.weightsPath = '../weights_justBlank2/my-v3_501000.weights'  # 模型权重文件
#         self.configPath = "../weights_justBlank2/my-v3.cfg"  # 模型配置文件
#         self.labelsPath = "../weights_justBlank2/classes.names"  # 模型类别标签文件
...
def Detect(net,LABELS,COLORS,image_path) -> object:
    ...
    for output in layerOutputs:  # 对三个输出层 循环
        for detection in output:  # 对每个输出层中的每个检测框循环
            ...
            if confidence > 0.55:  # 过滤掉那些置信度较小的检测结果
